...
def synthetic_data():
    ...
    np.random.seed(0)
    hierarchical_columns = ['poc_id','sku_id', 'region_id', 'country_id', 'city_id', 'order_id', 'channel_id', 'brand_id', 'subsegment_id', 'state_id','segment_id', 'route_id',' delivery_center_id', 'deliver_region_id', 'sales_route_id']
    date_columns = ['date', 'join_date', 'bees_join_date']
    integer_columns = ['days_since_last_order', 'is_purchased']
    float_columns = ['quantity','average_products_per_order', 'interaction_score', 'number_of_orders_for_pid', 'credit_total', 'credit_payment_terms', 'maltas', 'cervezas', 'agua', 'num_skus', 'num_orders', 'skus_per_order', 'average_order_revenue']
    df = pd.DataFrame(columns=hierarchical_columns+date_columns+integer_columns+float_columns)
    df[hierarchical_columns] = np.random.randint(1, 100, size=(1000, len(hierarchical_columns))).astype(str)
...
def download_file_from_blob(file_path, blob_path):
    ...
    blob_client = blob_service_client.get_blob_client(container=config['azure']['container_name'], blob=blob_path)
...
def read_sample_data(data_directory, file_name, n_rows, select_columns, date_configs, file_type, file_compression_type = None):
    if file_type == 'csv':
        df = pd.read_csv(os.path.join(data_directory, file_name), nrows=n_rows, usecols=select_columns, compression=file_compression_type)
...
def generate_optimized_data_schema(data, io_config, parsing_config):
    ...
    for date_col in expected_date_columns:
        data[date_col] = pd.to_datetime(data[date_col], format=parsing_config["date_format_configs"][date_col])
    actual_numeric_columns = data.select_dtypes(include=['int64', 'float64', 'int32', 'float32','int16', 'float16','int8']).columns.tolist()
    actual_categorical_columns = data.select_dtypes(include=['object', 'category']).columns.tolist()
    expected_numeric_columns = io_config['data_type_dict']['number']
    excepted_categorical_columns = io_config['data_type_dict']['category']
    missing_numeric_columns = list(set(expected_numeric_columns) - set(actual_numeric_columns))
    extra_numeric_columns = list(set(actual_numeric_columns) - set(expected_numeric_columns))
    missing_categorical_columns = list(set(excepted_categorical_columns) - set(actual_categorical_columns))
    extra_categorical_columns = list(set(actual_categorical_columns) - set(excepted_categorical_columns))
    ...
    data.loc[:,expected_numeric_columns] = optimize_numeric_data_types(data[expected_numeric_columns])
    data.loc[:,excepted_categorical_columns] = optimize_categorical_data_types(data[excepted_categorical_columns])
    float_columns = data.select_dtypes(include=[float]).columns.tolist()
    data[float_columns] = data[float_columns].round(parsing_config["output_data_precision"])
    return data

def read_full_data(data_directory, file_name, select_columns, date_configs, optimized_schema, file_type, file_compression_type):
    if file_type == "csv":
        df = pd.read_csv(os.path.join(data_directory, file_name), usecols=select_columns, compression=file_compression_type, dtype = optimized_schema)
        for date_col, date_format in date_configs.items():
            df[date_col] = pd.to_datetime(df[date_col], format=date_format)
        return df
...
def check_split_character_in_model_id_constructor(data, model_id_constructor, split_character):
...
def create_model_id(data, model_id, model_id_constructor, split_character):
    ...
    data[model_id] = data[model_id_constructor].astype(str).apply(lambda x: split_character.join(x), axis=1)
...
def extract_date_range(data, date_cols):
    ...
    for col in date_cols:
        date_range[col] = {"min_date": data[col].min().strftime("%Y-%m-%d"), "max_date" :data[col].max().strftime("%Y-%m-%d")}
    return date_range
...
if __name__ == '__main__':
    ...
    io_config = {
        ...
        "data_type_dict": {"number": ['quantity'], "category": ['poc_id','sku_id'], "date": ["date"]}
    }
    parsing_config = {
    ...
    validator_config = {
        ...
        "unique_check": {"poc_id":[45, 66, 48, 32, 80, 15, 21,  1, 59, 39, 73, 78, 86,  2, 33,  6, 41,
       13, 44, 43,  9, 19, 95,  8, 24, 54, 37, 36, 96, 51,  5, 52, 16, 94,
       65,  7, 46, 72, 34, 26, 88, 25, 35, 11, 49, 57, 55, 30, 81, 89, 82,
       12, 17, 83,  4, 76, 67, 38, 93, 40, 77, 53, 99, 28, 60, 56, 92, 47,
       10, 23, 63, 42, 58, 62, 75, 29, 31, 74, 50, 68, 90, 64, 61, 22, 18,
       70, 91, 79, 20, 69, 97, 14, 98,  3, 84, 87, 71, 27, 85], "sku_id":[48, 40, 65, 75,  5, 54, 59, 51, 24, 42, 20, 76, 32, 96, 12, 10, 37,
       45, 84, 87, 15, 80, 35, 60, 81, 25, 52, 69, 70,  1, 90, 46, 78, 47,
       67, 19, 50, 53, 27,  7, 44, 41, 94,  8, 63, 92, 82,  6, 61, 22, 99,
       89,  2, 43, 18, 28, 17, 31, 93, 91, 34, 62, 33, 23, 66, 30, 14, 64,
       26, 36, 83, 71, 97, 72, 88,  3, 73, 57,  9, 79, 29, 11, 39, 77, 98,
       95, 13, 55, 86, 16, 38, 58, 74, 21, 85, 56, 68,  4, 49]},
    ...
    processor_config = {
        "pre_processor": {
            ...
            "aggregation_configs": {},
            "column_mapper": {"a": "a", "b": "b", "c": "c", "d": "d", "e": "e", "f": "f", "g": "g", "h": "h", "i": "i"},
        },
        "post_processor": {
            "reverse_column_mapper": {}
        }
    }
    metadata_config = {"hash_name": "md5"}
    processor_config["post_processor"]["reverse_column_mapper"] = {v: k for k, v in processor_config["pre_processor"]["column_mapper"].items()}

    # Logging
    logger.add(os.path.join(io_config['log_directory'], 'data_preparation.log'), rotation="10 MB", level="INFO")
    # Create folders
    folder_list = ['data', 'data/raw', 'data/interim', 'data/processed', 'data/cached', 'configs', 'logs']
    create_folders(folder_list)
    files_list = ['data/raw/raw_data.csv', 'data/raw/test.csv', 'data/raw/sample_submission.csv']
    print(check_files_exists(files_list))
    df = synthetic_data()
    df.to_csv(os.path.join(io_config['data_directory'], 'raw', 'raw_data.csv'), index=False)
    print(df.head())
    show_config()
    sample_data = read_sample_data(os.path.join(io_config['data_directory'], 'raw'), 'raw_data.csv', 100, io_config['select_columns'], parsing_config['date_format_configs'], io_config['data_format'], io_config['compression'])
    print(sample_data.head())
    # FIXME: Next line is not working as expected. Why int64 is not getting converted to int32? Debug it.
    type_optimized_data = generate_optimized_data_schema(sample_data, io_config, parsing_config)
    print(type_optimized_data.head())
    optimized_data_schema = type_optimized_data.select_dtypes(include = ['number', 'category', 'bool', 'object']).dtypes.apply(lambda x: x.name).to_dict()
    print(optimized_data_schema)
    write_config(io_config['config_directory'], 'optimized_data_schema.json', optimized_data_schema, 'json')
    full_data = read_full_data(os.path.join(io_config['data_directory'], 'raw'), 'raw_data.csv', io_config['select_columns'], parsing_config['date_format_configs'], optimized_data_schema, io_config['data_format'], io_config['compression'])
    ...
    print(extract_date_range(full_data, list(parsing_config['date_format_configs'].keys())))
    print(extract_range(full_data, full_data.select_dtypes(include = ['number']).columns.tolist()))
    print(extract_unique_values(full_data, list(validator_config['unique_check'].keys()) + list(parsing_config['date_format_configs'].keys())))
    print(extract_unique_counts(full_data, list(validator_config['unique_check'].keys()) + list(parsing_config['date_format_configs'].keys())))
    if not check_model_id_in_data(full_data, processor_config['pre_processor']['model_id']):
        if check_model_id_constructor_in_data(full_data, processor_config['pre_processor']['model_id_constructor']):
            if not check_split_character_in_model_id_constructor(full_data, processor_config['pre_processor']['model_id_constructor'], processor_config['pre_processor']['model_split_character']):
                full_data = create_model_id(full_data, processor_config['pre_processor']['model_id'], processor_config['pre_processor']['model_id_constructor'], processor_config['pre_processor']['model_split_character'])
